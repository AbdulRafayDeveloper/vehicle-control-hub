...
import { TOGGLE_SIDEBAR_HOVER } from '../../../core/store/app/appSlice';
...
export default function Sidebar({ list = [] }) {
  ...
  const handleItemClick = index => {
    dispatch(TOGGLE_SIDEBAR_HOVER(false));
    setOpen(prev =>
      prev.map((p, _index) => ({
        ...p,
...
useEffect(() => {
    setOpen(prev =>
        prev.map(pre => ({
            ...pre,
            open: pre.path === pathname ||
                (Array.isArray(pre?.subItems) &&
                pre.subItems.some(subItem => {
                    const regex = createRegex(subItem.path);
